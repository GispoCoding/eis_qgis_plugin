...
class GraphicsScene(QtWidgets.QGraphicsScene):
    ...
    **Signals**
    ...
    def __init__(self, clickRadius=2, moveDistance=5, parent=None):
        ...
        self.contextMenu = [
            QtGui.QAction(
                QtCore.QCoreApplication.translate("GraphicsScene", "Export..."), self
            )
        ]
        ...
        if (
            ...
            for i in items:
                if (
                    i.isEnabled()
                    and i.isVisible()
                    and (i.flags() & i.GraphicsItemFlag.ItemIsFocusable)
                ):
                    ...
                    for btn in [
                        ...
                        if (
                            ...
                            if cev:
                                ...
                                if dist == 0 or (
                                    dist < self._moveDistance
                                    and now - cev.time() < self.minDragTime
                                ):
                                    continue
                                init = init or (
    ...
    def sendHoverEvents(self, ev, exitOnly=False):
        ...
        if ev.type() == ev.Type.GraphicsSceneMousePress or (
            ...
            self.lastHoverEvent = (
                event  ## save this so we can ask about accepted events later.
            )
    ...
    def sendDragEvent(self, ev, init=False, final=False):
        ...
        event = MouseDragEvent(
            ev, self.clickEvents[0], self.lastDrag, start=init, finish=final
        )
    ...
    def addParentContextMenus(self, item, menu, event):
        ...
        **Arguments:**
        ...
        while item is not self:
            ...
            if isinstance(
                subMenus, list
            ):  ## so that some items (like FlowchartViewBox) can return multiple menus
