...
class SVGExporter(Exporter):
        ...
        self.params = Parameter(
            ...
            children=[
                ...
                # {'name': 'normalize coordinates', 'type': 'bool', 'value': True},
                {
                    ...
                    "type": "bool",
                    "value": False,
                    "tip": "If False, strokes are non-scaling, "
    ...
    def export(self, fileName=None, toBytes=False, copy=False):
        if toBytes is False and copy is False and fileName is None:
            self.fileSaveDialog(
                filter=f"{translate('Exporter', 'Scalable Vector Graphics')} (*.svg)"
            )
            return
...
def generateSvg(item, options={}):
    ...
    backgroundtag = (
        '<rect width="100%%" height="100%%" style="fill:rgba(%d, %d, %d, %f)" />\n'
        % (c.red(), c.green(), c.blue(), c.alphaF())
    )
    return (
        (xmlHeader % svgAttributes)
        + backgroundtag
        + defsXml
...
def _generateItemSvg(item, nodes=None, root=None, options={}):
    ...
    if not isinstance(item, QtWidgets.QGraphicsScene):
        ...
        if item.flags() & item.GraphicsItemFlag.ItemClipsChildrenToShape:
            ...
            try:
                pathNode = _generateItemSvg(path, root=root, options=options)[
                    0
    ...
    if item.flags() & item.GraphicsItemFlag.ItemIgnoresTransformations:
        ...
        while True:
            ...
            if nextRoot is None:
                nextRoot = root
                break
            if nextRoot is root or (
                nextRoot.flags() & nextRoot.GraphicsItemFlag.ItemIgnoresTransformations
            ):
                break
