...
class PlotItem(GraphicsWidget):
    ...
    **Signals:**
    ...
    sigYRangeChanged = QtCore.Signal(
        object, object
    )  ## Emitted when the ViewBox Y range has changed
    sigXRangeChanged = QtCore.Signal(
        object, object
    ...
    def __init__(
        ...
        *axisItems*     Optional dictionary instructing the PlotItem to use pre-constructed items
        ...
        self.buttonsHidden = (
            False  ## whether the user has requested buttons to be hidden
        )
    ...
    def showGrid(self, x=None, y=None, alpha=None):
        ...
        **Arguments:**
        ...
        if alpha is not None:
            v = (
                fn.clip_scalar(alpha, 0, 1) * self.ctrl.gridAlphaSlider.maximum()
    ...
    def addAvgCurve(self, curve):
        ...
        if self.ctrl.avgParamList.count() > 0:
            ...
            if (
                len(remKeys) < 1
            ):  ## In this case, there would be 1 average plot for each data plot; not useful.
                return
    ...
    def writeSvgCurves(self, fileName=None):
        ...
        with open(fileName, "w") as fh:
            ...
            fh.write("<svg>\n")
            fh.write(
                '<path fill="none" stroke="#000000" stroke-opacity="0.5" '
    ...
    def downsampleMode(self):
        ...
        if self.ctrl.subsampleRadio.isChecked():
            ...
            raise ValueError(
                "one of the method radios must be selected for: 'subsample', 'mean', or 'peak'."
            )
    ...
    def _checkScaleKey(self, key):
        if key not in self.axes:
            raise Exception(
                "Scale '%s' not found. Scales are: %s"
                % (key, str(list(self.axes.keys())))
            )
    ...
    def showAxes(self, selection, showValues=True, size=False):
        ...
        if (
            showValues is True
        ):  # shortcut: defaults arrangement with labels at left and bottom
        ...
        if size is not False and not isinstance(size, collections.abc.Sized):
            size = (
                size,
                size,
            )  # make sure that size is either False or a full set of (width, height)
