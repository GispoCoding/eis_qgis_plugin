    ...
    **Signals:**
    ...
    def __init__(
        ...
        (
            self.lo_prv,
            self.hi_prv,
        ) = self.values  # remember previous values while adjusting range
        self.lo_lim = None
        self.hi_lim = None
    ...
    def setImageItem(self, img, insert_in=None):
        ...
        if insert_in is not None:
            if self.horizontal:
                insert_in.layout.addItem(
                    self, 5, 1
                )  # insert in layout below bottom axis
                insert_in.layout.setRowFixedHeight(
                    4, 10
                )  # enforce some space to axis above
            else:
                insert_in.layout.addItem(
                    self, 2, 5
                )  # insert in layout after right-hand axis
                insert_in.layout.setColumnFixedWidth(
                    4, 5
                )  # enforce some space to axis on the left
        ...
        for img_weakref in self.img_list:
            ...
            if update_cmap and self._colorMap is not None:
                if isinstance(img, PColorMeshItem):
                    img.setLookupTable(
                        self._colorMap.getLookupTable(
                            nPts=256, mode=self._colorMap.QCOLOR
                        )
                    )
                else:
    ...
    def _regionChanging(self):
        ...
        bot = math.copysign(
            bot**2, bot
        )  # quadratic behaviour for sensitivity to small changes
        ...
        hi_new = (
            self.hi_prv + (span_prv + 2 * self.rounding) * top
        )  # make sure that we can always
        lo_new = (
            self.lo_prv + (span_prv + 2 * self.rounding) * bot
        )  # reach 2x the minimal step
        ...
        if self.hi_lim is not None:
            if hi_new > self.hi_lim:  # limit maximum value
                ...
                if top != 0 and bot != 0:  # moving entire region?
                    lo_new = (
                        hi_new - span_prv
                    )  # avoid collapsing the span against top limit
        if self.lo_lim is not None:
            if lo_new < self.lo_lim:  # limit minimum value
                ...
                if top != 0 and bot != 0:  # moving entire region?
                    hi_new = (
                        lo_new + span_prv
                    )  # avoid collapsing the span against bottom limit
        if (
            hi_new - lo_new < self.rounding
        ):  # do not allow less than one "rounding" unit of span
            if bot == 0:
                hi_new = lo_new + self.rounding
            elif top == 0:
