...
class GraphicsItem(object):
    ...
    **Bases:** :class:`object`
    ...
    def __init__(self):
        ...
        if not hasattr(self, "_qtBaseClass"):
            raise Exception(
                "Could not determine Qt base class for GraphicsItem: %s" % str(self)
            )
    ...
    def deviceTransform(self, viewportTransform=None):
        ...
        if (
            dt.determinant() == 0
        ):  ## occurs when deviceTransform is invalid because widget has not been displayed
            return None
        else:
            return dt
    ...
    def informViewBoundsChanged(self):
        ...
        if (
            view is not None
            and hasattr(view, "implements")
            and view.implements("ViewBox")
        ):
            view.itemBoundsChanged(
                self
            )  ## inform view so it can update its range if it wants
