...
class Bessel(CtrlNode):
    ...
    def processData(self, data):
        ...
        if s["band"] == "lowpass":
            ...
            mode = "high"
        return functions.besselFilter(
            data, bidir=s["bidir"], btype=mode, cutoff=s["cutoff"], order=s["order"]
        )
...
class RemoveBaseline(PlottingCtrlNode):
    ...
    def processData(self, data):
        ...
        pts, indices = self.adjustXPositions(
            pts, timeVals
        )  ## maxe sure x positions match x positions of data points
...
class AdaptiveDetrend(CtrlNode):
    ...
    def processData(self, data):
        return functions.adaptiveDetrend(
            data, threshold=self.ctrls["threshold"].value()
        )
...
class HistogramDetrend(CtrlNode):
    ...
    uiTemplate = [
        (
            "windowSize",
            "intSpin",
            {"value": 500, "min": 10, "max": 1000000, "suffix": "pts"},
        ),
    ...
    def processData(self, data):
        ...
        return functions.histogramDetrend(
            data, window=s["windowSize"], bins=s["numBins"], offsetOnly=s["offsetOnly"]
        )
...
class RemovePeriodic(CtrlNode):
    ...
    uiTemplate = [
        ...
        (
            "f0",
            "spin",
            {"value": 60, "suffix": "Hz", "siPrefix": True, "min": 0, "max": None},
        ),
