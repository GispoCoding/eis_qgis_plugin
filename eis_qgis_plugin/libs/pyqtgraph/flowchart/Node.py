...
class Node(QtCore.QObject):
    ...
    def __init__(
        ...
        **Arguments:**
        ...
        self._allowAddInput = (
            allowAddInput  ## flags to allow the user to add/remove terminals
        )
        ...
        try:
            ...
            if signal:
                ...
                self.sigOutputChanged.emit(
                    self
                )  ## triggers flowchart to propagate new data
...
class NodeGraphicsItem(GraphicsObject):
    def __init__(self, node):
        ...
        self.nameItem.moveBy(
            self.bounds.width() / 2.0 - self.nameItem.boundingRect().width() / 2.0, 0
        )
    ...
    def labelChanged(self):
        ...
        self.nameItem.setPos(
            bounds.width() / 2.0 - self.nameItem.boundingRect().width() / 2.0, 0
        )
    ...
    def buildMenu(self):
        ...
        a = self.menu.addAction(
            translate("Context Menu", "Add input"), self.addInputFromMenu
        )
        if not self.node._allowAddInput:
            a.setEnabled(False)
        a = self.menu.addAction(
            translate("Context Menu", "Add output"), self.addOutputFromMenu
        )
        if not self.node._allowAddOutput:
            a.setEnabled(False)
        a = self.menu.addAction(
            translate("Context Menu", "Remove node"), self.node.close
        )
        if not self.node._allowRemove:
            a.setEnabled(False)
