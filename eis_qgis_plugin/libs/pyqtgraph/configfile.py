...
def genString(data, indent=""):
    ...
    for k in data:
        ...
        if sk[0] == " " or ":" in sk:
            print(data)
            raise Exception(
                'dict keys must not contain ":" or start with spaces [offending key is "%s"]'
                % sk
            )
...
def parseString(lines, start=0, **scope):
    ...
    if isinstance(lines, str):
        ...
        lines = lines.split("\n")
        lines = [
            l for l in lines if re.search(r"\S", l) and not re.match(r"\s*#", l)
        ]  ## remove empty lines
    ...
    try:
        while True:
            ...
            if lineInd > indent:
                ...
                raise ParseError(
                    "Indentation is incorrect. Expected %d, got %d" % (indent, lineInd),
                    ln + 1,
                    l,
                )
            ...
            if re.search(r"\S", v) and v[0] != "#":  ## eval the value
                try:
                    ...
                    ex = sys.exc_info()[1]
                    raise ParseError(
                        "Error evaluating expression '%s': [%s: %s]"
                        % (v, ex.__class__.__name__, str(ex)),
                        (ln + 1),
                        l,
                    )
            else:
