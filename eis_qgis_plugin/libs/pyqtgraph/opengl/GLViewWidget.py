...
class GLViewWidget(QtWidgets.QOpenGLWidget):
    ...
    def initializeGL(self):
        ...
        if ctx.isOpenGLES() or fmt.version() < (2, 0):
            ...
            raise RuntimeError(
                "pyqtgraph.opengl: Requires >= OpenGL 2.0 (not ES); Found %s"
                % verString
            )
    ...
    def itemsAt(self, region=None):
        ...
        region = (
            region[0],
            self.deviceHeight() - (region[1] + region[3]),
            region[2],
            region[3],
        )
    ...
    def cameraPosition(self):
        ...
        if self.opts["rotationMethod"] == "quaternion":
            pos = Vector(
                center - self.opts["rotation"].rotatedVector(Vector(0, 0, dist))
            )
        else:
    ...
    def orbit(self, azim, elev):
        ...
        if self.opts["rotationMethod"] == "quaternion":
            ...
            self.opts["elevation"] = fn.clip_scalar(
                self.opts["elevation"] + elev, -90.0, 90.0
            )
        self.update()
    ...
    def pan(self, dx, dy, dz, relative="global"):
        ...
        *relative*      String that determines the direction of dx,dy,dz.
        ...
        if relative == "global":
            ...
            xDist = (
                dist * 2.0 * tan(0.5 * radians(self.opts["fov"]))
    ...
    def renderToArray(
        self, size, format=GL_BGRA, type=GL_UNSIGNED_BYTE, textureSize=1024, padding=256
    ):
        w, h = map(int, size)
        ...
        try:
            ...
            glRenderbufferStorage(
                GL_RENDERBUFFER, GL_DEPTH_COMPONENT, texwidth, texwidth
            )
            ...
            for x in range(-padding, w - padding, texwidth - p2):
                for y in range(-padding, h - padding, texwidth - p2):
                    ...
                    data = np.frombuffer(data, dtype=np.ubyte).reshape(
                        texwidth, texwidth, 4
                    )[::-1, ...]
                    output[
                        y + padding : y2 - padding, x + padding : x2 - padding
                    ] = data[-(h2 - padding) : -padding, padding : w2 - padding]
