...
class MeshData(object):
    ...
    def __init__(
        self, vertexes=None, faces=None, edges=None, vertexColors=None, faceColors=None
    ):
        """
        ==============  =====================================================
        **Arguments:**
        ...
        self._vertexFaces = (
            None  ## maps vertex ID to a list of face IDs (inverse mapping of _faces)
        )
        self._vertexEdges = (
            None  ## maps vertex ID to a list of edge IDs (inverse mapping of _edges)
        )
    ...
    def sphere(rows, cols, radius=1.0, offset=True):
        ...
        if offset:
            th = th + (
                (np.pi / cols) * np.arange(rows + 1).reshape(rows + 1, 1)
        ...
        rowtemplate2 = (
            (np.arange(cols).reshape(cols, 1) + np.array([[0, 1, 1]])) % cols
    ...
    def cylinder(rows, cols, radius=[1.0, 1.0], length=1.0, offset=False):
        ...
        verts[..., 2] = np.linspace(0, length, num=rows + 1, endpoint=True).reshape(
            rows + 1, 1
        )  # z
        if offset:
            th = th + (
