...
class GraphicsView(QtWidgets.QGraphicsView):
    ...
    def setAntialiasing(self, aa):
        ...
        if aa:
            self.setRenderHints(
                self.renderHints() | QtGui.QPainter.RenderHint.Antialiasing
            )
        else:
            self.setRenderHints(
                self.renderHints() & ~QtGui.QPainter.RenderHint.Antialiasing
    ...
    def useOpenGL(self, b=True):
        if b:
            ...
            if not HAVE_OPENGL:
                raise Exception(
                    "Requested to use OpenGL with QGraphicsView, but QOpenGLWidget is not available."
                )
    ...
    def setRange(
        ...
        scaleChanged = False
        if (
            self.range.width() != newRect.width()
            or self.range.height() != newRect.height()
        ):
            scaleChanged = True
        self.range = newRect
    ...
    def mouseMoveEvent(self, ev):
        ...
        if ev.buttons() == QtCore.Qt.MouseButton.RightButton:
            delta = Point(
                fn.clip_scalar(delta[0], -50, 50), fn.clip_scalar(-delta[1], -50, 50)
            )
            scale = 1.01**delta
