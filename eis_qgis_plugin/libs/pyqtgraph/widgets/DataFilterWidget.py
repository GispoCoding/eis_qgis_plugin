...
class DataFilterParameter(ptree.types.GroupParameter):
    ...
    def __init__(self):
        ...
        ptree.types.GroupParameter.__init__(
            self, name="Data Filter", addText="Add filter..", addList=[]
        )
    ...
    def setEnumVals(self, opts):
        ...
        for val, valopts in vals.items():
            ...
            ch = ptree.Parameter.create(
                name=vname, type="bool", value=prevState.get(vname, enabled)
            )
            ch.maskValue = val
            self.addChild(ch)
        ch = ptree.Parameter.create(
            name="(other)", type="bool", value=prevState.get("(other)", True)
        )
