...
class ScatterPlotWidget(QtWidgets.QSplitter):
    ...
    def updatePlot(self):
        ...
        units = list(
            [item.opts.get("units", "") for item in self.fieldList.selectedItems()]
        )
        if len(sel) == 0:
            self.plot.setTitle("")
            return
        ...
        for i in [0, 1]:
            ...
            if xy[i] is not None and (
                self.fields[sel[i]].get("mode", None) == "enum"
                or xy[i].dtype.kind in ("S", "O")
            ):
                vals = self.fields[sel[i]].get("values", list(set(xy[i])))
                xy[i] = np.array(
                    [vals.index(x) if x in vals else len(vals) for x in xy[i]],
                    dtype=float,
                )
    ...
    def updateSelected(self):
        ...
        if len(x) == 0:
            return
        self.selectionScatter = self.plot.plot(
            x, y, pen=None, symbol="s", symbolSize=12, symbolBrush=None, symbolPen="y"
        )
