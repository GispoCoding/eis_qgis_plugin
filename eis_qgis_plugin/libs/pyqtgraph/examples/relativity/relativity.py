...
class RelativityGUI(QtWidgets.QWidget):
    def __init__(self):
        ...
        self.params.param("Recalculate Worldlines").sigActivated.connect(
            self.recalculate
        )
        ...
        presetDir = os.path.join(
            os.path.abspath(os.path.dirname(sys.argv[0])), "presets"
        )
    ...
    def recalculate(self):
        ...
        dur = clocks1[ref].refData["pt"][
            -1
        ]  ## decide how long to run the reference simulation
...
class ObjectGroupParam(pTypes.GroupParameter):
    def __init__(self):
        pTypes.GroupParameter.__init__(
            self, name="Objects", addText="Add New..", addList=["Clock", "Grid"]
        )
    ...
    def runReference(self, nPts):
        ...
        for i in range(1, nPts):
            ...
            while True:
                ...
                dtau = min(
                    nextPt - ref.pt, tau2 - ref.pt
                )  ## do not step past the next command boundary
                g = ref.acceleration()
            ...
            for cl in clocks.values():
                while True:
                    ...
                    t1 = Simulation.hypIntersect(
                        ref.x, ref.t, ref.v, cl.x, cl.t, cl.v, g
                    )
                    dt1 = t1 - cl.t
                ...
                cl.reft = (
                    ref.pt
                )  #  + gamma * (t - ref.v * x)   # this term belongs here, but it should always be equal to 0.
