...
view_auto_scale = win.addPlot(
    0, 0, 1, 1, title="Auto-scaling colorscheme", enableMenu=False
)
view_consistent_scale = win.addPlot(
    1, 0, 1, 1, title="Consistent colorscheme", enableMenu=False
)
...
bar_static = pg.ColorBarItem(
    label="Z value [arbitrary unit]", interactive=True, rounding=0.1
)
...
def updateData():
    ...
    new_z = (
        np.exp(-((x - np.cos(i * color_speed) * xn) ** 2) / 1000)[:-1, :-1]
        + color_noise
    )
