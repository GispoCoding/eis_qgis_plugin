...
class ParameterItem(QtWidgets.QTreeWidgetItem):
    ...
    def __init__(self, param, depth=0):
        ...
        self.param = param
        self.param.registerItem(
            self
        )  ## let parameter know this item is connected to it (for debugging)
        self.depth = depth
    ...
    def updateFlags(self):
        ...
        if opts.get("renamable", False):
            if opts.get("title", None) is not None:
                raise Exception(
                    "Cannot make parameter with both title != None and renamable == True."
                )
    ...
    def contextMenuEvent(self, ev):
        ...
        if (
            not opts.get("removable", False)
            and not opts.get("renamable", False)
        ...
        self.contextMenu = (
            QtWidgets.QMenu()
        )  # Put in global name space to prevent garbage collection
        self.contextMenu.addSeparator()
        if opts.get("renamable", False):
            self.contextMenu.addAction(
                translate("ParameterItem", "Rename")
            ).triggered.connect(self.editName)
        if opts.get("removable", False):
            self.contextMenu.addAction(
                translate("ParameterItem", "Remove")
            ).triggered.connect(self.requestRemove)
    ...
    def columnChangedEvent(self, col):
        ...
        if col == 0 and (self.param.opts.get("title", None) is None):
            ...
            try:
                ...
                self.nameChanged(
                    self, newName
                )  ## If the parameter rejects the name change, we need to set it back.
            finally:
                self.ignoreNameColumnChange = False
