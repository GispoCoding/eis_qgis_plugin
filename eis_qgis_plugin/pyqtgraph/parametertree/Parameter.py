...
def registerParameterItemType(name, itemCls, parameterCls=None, override=False):
    ...
    if name in _PARAM_ITEM_TYPES and not override:
        raise Exception(
            "Parameter item type '%s' already exists (use override=True to replace)"
            % name
        )
...
def registerParameterType(name, cls, override=False):
    ...
    if name in PARAM_TYPES and not override:
        raise Exception(
            "Parameter type '%s' already exists (use override=True to replace)" % name
        )
...
class Parameter(QtCore.QObject):
    ...
    **Signals:**
    ...
    sigValueChanged = QtCore.Signal(
        object, object
    )  ## self, value   emitted when value is finished being edited
    sigValueChanging = QtCore.Signal(
        object, object
    )  ## self, value  emitted as value is being edited
    ...
    def __init__(self, **opts):
        ...
        **Keyword Arguments:**
        ...
        )  ## keeps track of tree items representing this parameter
        self._parent = None
        self.treeStateChanges = (
            []
        )  ## cache of tree state changes to be delivered on next emit
    ...
    def setName(self, name):
        ...
        if parent is not None:
            name = parent._renameChild(
                self, name
            )  ## first ask parent if it's ok to rename
    ...
    def makeTreeItem(self, depth):
        ...
        itemClass = self.itemClass or _PARAM_ITEM_TYPES.get(
            self.opts["type"], ParameterItem
        )
    ...
    def insertChild(self, pos, child, autoIncrementName=None, existOk=False):
        ...
        if name in self.names and child is not self.names[name]:
            if autoIncrementName is True or (
                autoIncrementName is None and child.opts.get("autoIncrementName", False)
            ):
    ...
    def child(self, *names):
        ...
        try:
            ...
            raise KeyError(
                "Parameter %s has no child named %s" % (self.name(), names[0])
            )
