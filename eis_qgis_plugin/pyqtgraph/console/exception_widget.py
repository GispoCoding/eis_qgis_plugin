...
class ExceptionHandlerWidget(QtWidgets.QGroupBox):
    ...
    def _setupUi(self):
        ...
        self.runSelectedFrameCheck = QtWidgets.QCheckBox(
            "Run commands in selected stack frame", self
        )
    ...
    def catchAllExceptions(self, catch=True):
        ...
        if catch:
            with SignalBlock(
                self.catchNextExceptionBtn.toggled, self.catchNextException
            ):
    ...
    def catchNextException(self, catch=True):
        ...
        if catch:
            with SignalBlock(
                self.catchAllExceptionsBtn.toggled, self.catchAllExceptions
            ):
    ...
    def updateSysTrace(self):
        ...
        if (
            not self.catchNextExceptionBtn.isChecked()
            and not self.catchAllExceptionsBtn.isChecked()
        ):
