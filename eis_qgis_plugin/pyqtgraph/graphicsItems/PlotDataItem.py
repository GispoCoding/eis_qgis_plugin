...
class PlotDataset(object):
    ...
    def _updateDataRect(self):
        ...
        if self.containsNonfinite is False:  # all points are directly usable.
            ...
            all_y_are_finite = (
                selection.all()
            )  # False if all values are finite, True if there are any non-finites
            try:
            ...
            self.containsNonfinite = not (
                all_x_are_finite and all_y_are_finite
            )  # This always yields a definite True/False answer
    ...
    def applyLogMapping(self, logMode):
        ...
        if all_x_finite and all_y_finite:
            self.containsNonfinite = (
                False  # mark as False only if both axes were checked.
            )
            ...
            if view_range is not None:
                ...
                if data_range is not None:
                    ...
                    if (  # note that "bottom" is the larger number, and "top" is the smaller one.
                        view_height
                        > 0  # never clip if the view does not show anything and would cause division by zero
                        and not data_range.bottom()
                        < view_range.top()  # never clip if all data is too small to see
                        and not data_range.top()
                        > view_range.bottom()  # never clip if all data is too large to see
                        ...
                        if (
                            ...
                            top_exc = (
                                -(self._drlLastClip[0] - view_range.bottom())
                                / view_height
                            )
                            bot_exc = (
        ...
        **Arguments:**
        ...
        if self.curve.isVisible():
            ...
            range = [
                r2[0]
                if range[0] is None
                else (range[0] if r2[0] is None else min(r2[0], range[0])),
                r2[1]
                if range[1] is None
                else (range[1] if r2[1] is None else min(r2[1], range[1])),
            ]
        return range
    ...
    if isinstance(obj, dict):
        ...
        if hasattr(obj, "implements") and obj.implements("MetaArray"):
            ...
            if obj.ndim == 1:
                ...
                raise ValueError(
                    "array shape must be (N,) or (N,2); got %s instead" % str(obj.shape)
                )
