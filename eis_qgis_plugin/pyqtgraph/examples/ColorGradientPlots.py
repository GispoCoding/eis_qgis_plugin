...
class MainWindow(pg.GraphicsLayoutWidget):
        ...
        self.traces = (
            {
                ...
                "buf": np.zeros(length),
                "ptr": 0,
                "ds": DataSource(signal_period=0.55),
            },
            {
                "crv": curve1,
                "buf": np.zeros(length),
                "ptr": 0,
                "ds": DataSource(signal_period=0.61, negative_period=0.55),
            },
            {
                "crv": curve2,
                "buf": np.zeros(length),
                "ptr": 0,
                "ds": DataSource(signal_period=0.65),
            },
            {
                "crv": curve3,
                "buf": np.zeros(length),
                "ptr": 0,
                "ds": DataSource(signal_period=0.52),
            },
        )
    ...
    def update(self):
        ...
        if self.mean_dt is None:
            ...
            self.mean_dt = (
                0.95 * self.mean_dt + 0.05 * dt
            )  # average over fluctuating measurements
        self.top_plot.setTitle(
            "refresh: {:0.1f}ms -> {:0.1f} fps".format(
        ...
        for dic in self.traces:
            ...
            if idx_b < len_buffer:  # data does not cross buffer boundary
                ...
                dic["buf"][idx_a : idx_a + len_1] = new_data[
                    :len_1
                ]  # first part of data at end
                idx_b = len(new_data) - len_1
                dic["buf"][0:idx_b] = new_data[
                    len_1:
                ]  # second part of data at re-start
            dic["ptr"] = idx_b
