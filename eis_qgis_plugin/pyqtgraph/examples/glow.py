...
params = pg.parametertree.Parameter.create(
    ...
    for color, x, y in zip(colors, xs, ys):
        ...
        if params.child("add_underglow").value() == "Full":
            kw = {
                ...
                "fillBrush": pg.mkBrush(
                    color="{}{:02x}".format(color, alpha_underglow)
                ),
            }
            ...
            grad.setColorAt(
                abs(y.min()) / (y.max() - y.min()),
                pg.mkColor("{}{:02x}".format(color, alpha_underglow)),
            )
        ...
        if params.child("make_line_glow").value():
            ...
            for alpha, lw in zip(alphas, lws):
                ...
                pen = pg.mkPen(
                    color="{}{:02x}".format(color, alpha), width=lw, connect="finite"
                )
