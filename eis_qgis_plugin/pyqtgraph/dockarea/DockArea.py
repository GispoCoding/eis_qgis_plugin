        ...
        **Arguments:**
        ...
        if relativeTo is None or relativeTo is self:
            ...
            if container is None:
                raise TypeError(
                    "Dock %s is not contained in a DockArea; cannot add another dock relative to it."
                    % relativeTo
                )
            neighbor = relativeTo
    ...
    def buildFromState(self, state, docks, root, depth=0, missing="error"):
        ...
        if typ == "dock":
            try:
                ...
                if missing == "error":
                    raise Exception(
                        'Cannot restore dock state; no dock with name "%s"' % contents
                    )
                elif missing == "create":
                    ...
                    raise ValueError(
                        '"missing" argument must be one of "error", "create", or "ignore".'
                    )
