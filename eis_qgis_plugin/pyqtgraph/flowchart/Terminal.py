...
class Terminal(object):
    def __init__(
        ...
        **Arguments:**
        ...
        self._connections = {}
        self._graphicsItem = TerminalGraphicsItem(
            self, parent=self._node().graphicsItem()
        )
        self._bypass = bypass
    ...
    def recolor(self, color=None, recurse=True):
        if color is None:
            if not self.isConnected():  ## disconnected terminals are black
                ...
                self.isInput() and not self.hasInput()
                ...
                self.valueIsAcceptable() is None
...
class TerminalGraphicsItem(GraphicsObject):
    ...
    def setAnchor(self, x, y):
        ...
        if self.term.isInput():
            ...
            self.label.setPos(
                pos.x() - br.width() - lr.width(), pos.y() - lr.height() / 2.0
            )
    ...
    def getMenu(self):
        if self.menu is None:
            ...
            remAct = QtGui.QAction(
                translate("Context Menu", "Remove terminal"), self.menu
            )
            ...
            multiAct = QtGui.QAction(
                translate("Context Menu", "Multi-value"), self.menu
            )
    ...
    def connectPoint(self):
        ...
        return self.mapToView(
            self.mapFromItem(self.box, self.box.boundingRect().center())
        )
...
class ConnectionItem(GraphicsObject):
    ...
    def generatePath(self, start, stop):
        ...
        if self.style["shape"] == "line":
            ...
            raise Exception(
                'Invalid shape "%s"; options are "line" or "cubic"'
                % self.style["shape"]
            )
        return path
    ...
    def keyPressEvent(self, ev):
        ...
        if (
            ev.key() == QtCore.Qt.Key.Key_Delete
            or ev.key() == QtCore.Qt.Key.Key_Backspace
        ):
    ...
    def paint(self, p, *args):
        if self.isSelected():
            p.setPen(
                fn.mkPen(self.style["selectedColor"], width=self.style["selectedWidth"])
            )
        else:
            if self.hovered:
                p.setPen(
                    fn.mkPen(self.style["hoverColor"], width=self.style["hoverWidth"])
                )
            else:
