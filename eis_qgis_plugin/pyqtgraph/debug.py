...
if sys.version.startswith("3.8") and QT_LIB == "PySide2":
    ...
    if tuple(map(int, PySide2.__version__.split("."))) < (
        5,
        14,
...
def findRefPath(
    startObj, endObj, maxLen=8, restart=True, seen=None, path=None, ignore=None
):
    ...
    for r in newRefs:
        ...
        if tree is None:
            tree = findRefPath(
                startObj, r, maxLen - 1, restart=False, path=[r] + path, ignore=ignore
            )
    ...
    try:
        if type(obj) in [int, float]:
            ...
            if len(obj) > 5:
                return "<%s [%s,...]>" % (
                    type(obj).__name__,
                    ",".join([type(o).__name__ for o in obj[:5]]),
                )
            else:
                return "<%s [%s]>" % (
                    type(obj).__name__,
                    ",".join([type(o).__name__ for o in obj]),
                )
        else:
...
class Profiler(object):
    ...
    def finish(self, msg=None):
        ...
        self._newMsg(
            ...
            self._name,
            (perf_counter() - self._firstTime) * 1000,
        )
        type(self)._depth -= 1
        if self._depth < 1:
            self.flush()
...
class ObjTracker(object):
    ...
    def __init__(self):
        self.startRefs = (
            {}
        )  ## list of objects that exist when the tracker is initialized {oid: weakref}
        ##   (If it is not possible to weakref the object, then the value is None)
        self.startCount = {}
        self.newRefs = (
            {}
        )  ## list of objects that have been created since initialization
        self.persistentRefs = (
            {}
        )  ## list of objects considered 'persistent' when the last diff() was called
        self.objTypes = {}
    ...
    def diff(self, **kargs):
        ...
        for o in refs:
            if o not in self.startRefs:
                if o not in self.newRefs:
                    createRefs[o] = refs[
                        o
                    ]  ## object has been created since last diff()
                else:
        ...
        print(
            "-----------  %d Created since start (persistent): ------------"
            % len(persistentRefs)
        )
...
def describeObj(obj, depth=4, path=None, ignore=None):
    ...
    for ref in refs:
        ...
        if len(newPath) >= depth:
            ...
            if (
                "[_]" not in refStr
            ):  ## ignore '_' references generated by the interactive shell
                print(refStr)
            printed = True
        else:
