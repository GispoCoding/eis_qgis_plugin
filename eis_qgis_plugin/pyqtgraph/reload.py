...
def reloadAll(prefix=None, debug=False):
    ...
    for modName, mod in list(sys.modules.items()):
        ...
        try:
            ...
            ret[modName] = (
                False,
                "reload failed: %s" % traceback.format_exception_only(type(exc), exc),
            )
...
def updateFunction(old, new, debug, depth=0, visited=None):
    ...
    if hasattr(old, "__previous_reload_version__"):
        maxDepth = updateFunction(
            old.__previous_reload_version__,
            new,
            debug,
            depth=depth + 1,
            visited=visited,
...
def updateClass(old, new, debug):
    ...
    for ref in refs:
        try:
            ...
            print(
                "Error updating reference (%s) for class change (%s -> %s)"
    ...
    for attr in dir(old):
        ...
        if inspect.isfunction(oa) or inspect.ismethod(oa):
            ...
            ofunc = getattr(
                oa, "__func__", oa
            )  # in py2 we have to get the __func__ from unbound method,
            ...
            if ofunc is not nfunc:
                ...
                if not hasattr(nfunc, "__previous_reload_method__"):
                    nfunc.__previous_reload_method__ = (
                        oa  # important for managing signal connection
                    )
