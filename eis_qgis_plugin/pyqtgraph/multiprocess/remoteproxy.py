...
class RemoteEventHandler(object):
    ...
    handlers = (
        {}
    )  ## maps {process ID : handler}. This allows unpickler to determine which process
    ...
    def __init__(self, connection, name, pid, debug=False):
        ...
        self.results = (
            {}
        )  ## reqId: (status, result); cache of request results received from the remote process
        ...
        RemoteEventHandler.handlers[
            pid
        ] = self  ## register this handler as the one communicating with pid
        ...
        with self.processLock:
            ...
            while self.conn.poll():
                ...
                try:
                    ...
                    self.debugMsg(
                        "processRequests: got ClosedError from handleRequest; setting exited=True."
                    )
                    self.exited = True
                    raise
    ...
    def handleRequest(self):
        ...
        try:
            if cmd == "result" or cmd == "error":
                resultId = reqId
                reqId = (
                    None  ## prevents attempt to return information from this request
                )
            ...
            if cmd == "result":
                ...
                if len(byteData) > 0:
                    for i, arg in enumerate(fnargs):
                        if (
                            ...
                            and len(arg) > 0
                            and arg[0] == "__byte_message__"
                        ):
                            ind = arg[1]
                            dtype, shape = arg[2]
                            fnargs[i] = np.frombuffer(
                                byteData[ind], dtype=dtype
                            ).reshape(shape)
                ...
                result = np.frombuffer(byteData[0], dtype=opts["dtype"]).reshape(
                    opts["shape"]
                )
                returnType = "proxy"
            elif cmd == "import":
        ...
        if reqId is not None:
            if exc is None:
                self.debugMsg(
                    "    handleRequest: sending return value for %d: %s", reqId, result
                )
    ...
    def replyResult(self, reqId, result):
        self.send(
            request="result", reqId=reqId, callSync="off", opts=dict(result=result)
        )
    ...
    def replyError(self, reqId, *exc):
        ...
        try:
            self.send(
                ...
                reqId=reqId,
                callSync="off",
                opts=dict(exception=exc[1], excString=excStr),
            )
        except:
            self.send(
                request="error",
                reqId=reqId,
                callSync="off",
                opts=dict(exception=None, excString=excStr),
            )
        ...
        **Arguments:**
        ...
        with self.sendLock:
            ...
            if reqId is None:
                if (
                    callSync != "off"
                ):  ## requested return value; use the next available request ID
            ...
            self.debugMsg(
                "send request: cmd=%s nByteMsgs=%d id=%s opts=%s",
                request[0],
                nByteMsgs,
                reqId,
                opts,
            ...
            if byteData is not None:
                for (
                    ...
                    byteData
                ):  ## Remote process _must_ be prepared to read the same number of byte messages!
    ...
    def getResult(self, reqId):
        ...
        if status == "result":
            ...
            if exc is not None:
                ...
                print("".join(excStr))
                raise Exception(
                    "Error getting result. See above for exception from remote process."
                )
    ...
    def _import(self, mod, **kwds):
        ...
        """
        return self.send(
            request="import", callSync="sync", opts=dict(module=mod), **kwds
        )
    ...
    def callObj(self, obj, args, kwds, **opts):
        ...
        return self.send(
            request="callObj",
            opts=dict(obj=obj, args=args, kwds=kwds),
            byteData=byteMsgs,
            **opts
        )
...
class Request(object):
    ...
    def result(self, block=True, timeout=None):
        ...
        if block:
            ...
            self._result = self.proc.getResult(
                self.reqId
            )  ## raises NoResultError if result is not available yet
            self.gotResult = True
            return self._result
...
class LocalObjectProxy(object):
    ...
    def __reduce__(self):
        ...
        return (
            unpickleObjectProxy,
            (self.processId, pid, self.typeStr, None, self.opts),
        )
...
class ObjectProxy(object):
    ...
    def __reduce__(self):
        return (
            unpickleObjectProxy,
            (self._processId, self._proxyId, self._typeStr, self._attributes),
        )

    def __repr__(self):
        # objRepr = self.__getattr__('__repr__')(callSync='value')
        return "<ObjectProxy for process %d, object 0x%x: %s >" % (
            self._processId,
            self._proxyId,
            self._typeStr,
        )
