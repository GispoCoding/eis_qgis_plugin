...
class Parallelize(object):
    ...
    def __exit__(self, *exc_info):
        ...
        if self.proc is not None:  ## worker
            ...
            )  ## hit an exception during processing.
    ...
    def runParallel(self):
        ...
        for i in range(workers):
            proc = ForkedProcess(
                target=None, preProxy=self.kwds, randomReseed=self.reseed
            )
        ...
        try:
            ...
            while len(activeChilds) > 0:
                ...
                if waitingChildren > 1:
                    ...
                    pollInterval /= 0.7
                pollInterval = max(
                    min(pollInterval, 0.5), 0.0005
                )  ## but keep it within reasonable limits
