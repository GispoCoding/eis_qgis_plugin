...
class HDF5Exporter(Exporter):
    ...
    def export(self, fileName=None):
        if not HAVE_HDF5:
            raise RuntimeError(
                "This exporter requires the h5py package, " "but it was not importable."
            )
        ...
        fd = h5py.File(
            fileName, "a"
        )  # forces append to file... 'w' doesn't seem to "delete/overwrite"
        data = []

        appendAllX = self.params["columnMode"] == "(x,y) per plot"
        # Check if the arrays are ragged
        len_first = (
            len(self.item.curves[0].getData()[0]) if self.item.curves[0] else None
        )
