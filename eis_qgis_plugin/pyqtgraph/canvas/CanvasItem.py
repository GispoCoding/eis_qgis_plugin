...
class CanvasItem(QtCore.QObject):
    ...
    def __init__(self, item, **opts):
        ...
        self.selectBox = SelectBox(
            scalable=self.opts["scalable"], rotatable=self.opts["rotatable"]
        )
        self.selectBox.hide()
        self.selectBox.setZValue(1e6)
        self.selectBox.sigRegionChanged.connect(
            self.selectBoxChanged
        )  ## calls selectBoxMoved
    ...
    def applyTemporaryTransform(self):
        ...
        self.userTransform = (
            self.userTransform * self.tempTransform
        )  ## order is important!
    ...
    def updateTransform(self):
        ...
        transform = (
            self.baseTransform * self.userTransform * self.tempTransform
        )  ## order is important
