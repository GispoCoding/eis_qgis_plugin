...
class EISMlpClassifier(EISProcessingAlgorithm):
    ...
    def processAlgorithm(
        ...
        neurons_raw = self.parameterAsString(parameters, self.alg_parameters[neurons_param_i], context).lower()
        values = re.split(';|,', neurons_raw)
        neurons_options = []
        for value in values:
            neurons_options.append("--" + self.alg_parameters[neurons_param_i].replace("_", "-"))
            neurons_options.append(value) 

        # Remove breaks from the list to not prepare them again in the next step
        self.alg_parameters.pop(neurons_param_i)
        typer_args, typer_options, output_path = self.prepare_arguments(parameters, context)
        typer_options += neurons_options  # Combine lists
